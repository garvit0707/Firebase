...
const LoginScreen = () => {
  ...
  console.log('!!!!!!!!!!!!!!!!!!!!!!!!!1', user);
  const dataGet = async () => {
    const data = await getData('usercreddd');
  ...
  const handleLogin = () => {
    try {
      EventLog({
        ...
        payload: {
          name: 'garvitsharma',
          loginid: '2',
          loginemail: 'garvit.sharma.0707@gmail.com',
        },
      });
    } catch (error) {
    ...
    signInWithEmailAndPassword(getAuth(), email, Password)
      .then(Response => {
        ...
        console.log(
          'user details before sending is @@@@@@@@@@@@@@@@@@@@----------->>>>>>>>>>>>',
          Response?.user._user.providerData[0],
        );
    ...
    <View>
      ...
      <View style={style.signupTextContainer}>
        ...
        >
          {({ pressed }) => (
            <Text
              style={{ color: pressed ? 'blue' : 'black', fontWeight: '700' }}
            >
              Sign up
            </Text>
          )}
